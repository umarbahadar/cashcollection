@php
    // Optional business info (company logo / name)
    $business = null;
    $logoData = null;
...
<table>
    ...
    <tr>
        ...
        <td>
            @if($ledger)
                <span style="font-weight:bold; color:{{ $ledger->type == 'credit' ? 'green' : 'red' }}">
                    {{ ucfirst($ledger->type) }}
                </span>
            @else
                --
    ...
    <tr>
        <th>Ledger Amount</th>
        <td>{{ $ledger?->amount ? number_format($ledger->amount, 2) : '--' }}</td>
    </tr>
    <tr>
        <th>Description</th>
